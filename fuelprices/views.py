...
class FuelStationList(generics.ListCreateAPIView):
	...
	def get_queryset(self):
		...
		ordered_queryset = queryset.extra(select = {'farness': 'select ((latitude-(%f)) * (latitude-(%f))) + ((longitude-(%f)) * (longitude-(%f)))'
			%(float(latit), float(latit), float(longit), float(longit)),}, order_by = ['farness',])[:5]
...
def googleUsers(request):
	...
	if idinfo is not None:
		...
		try:
			...
			user_ = User.objects.create_user(username=email_, email=email_, first_name= first_name_, last_name=last_name_, password='#save#')
